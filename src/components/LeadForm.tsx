...
export function LeadForm({ onCourseSelect, onLeadSuccess, onPricingUpdate, leadData, setLeadData }: LeadFormProps) {
  ...
  const [onlineConsultantsData, setOnlineConsultantsData] = useState<any[]>([]);
      ...
      .on('presence', { event: 'sync' }, () => {
        ...
        for (const key in state) {
          if (key !== 'lead') {
             ...
             presences.forEach(p => {
               // Evita duplicar se o consultor abrir duas abas
               if (!consultants.find(c => c.nome === p.nome)) {
                 consultants.push(p);
               }
             });
          }
          ...
          {onlineConsultants > 0 && (
            <div className="bg-white px-5 py-4 rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,59,92,0.2)] border border-gray-100 flex flex-col gap-3 pointer-events-auto origin-bottom-right animate-in fade-in zoom-in-95 duration-500">
               <div className="flex items-center justify-between gap-6">
                 <div>
                   <h4 className="font-black text-[#003B5C] text-[15px] mb-0.5 tracking-tight">Dúvidas? Fale conosco!</h4>
                 ...
                 {/* Avatares dos consultores */}
                 <div className="flex items-center">
                   {onlineConsultantsData.slice(0, 3).map((consultant, idx) => (
                     <div key={idx} className={`w-10 h-10 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center overflow-hidden shadow-sm ${idx > 0 ? '-ml-3' : ''}`}>
                       {consultant.avatar_url ? (
                         <img src={consultant.avatar_url} alt={consultant.nome} className="w-full h-full object-cover" />
                       ) : (
                         <span className="text-[#003B5C] font-black text-[10px]">{consultant.nome?.substring(0, 2).toUpperCase()}</span>
                       )}
                     </div>
                   ))}
                   {onlineConsultantsData.length > 3 && (
                     <div className="w-10 h-10 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center shadow-sm -ml-3 z-10">
                       <span className="text-gray-600 font-black text-[10px]">+{onlineConsultantsData.length - 3}</span>
                     </div>
                   )}
                 </div>
               </div>
